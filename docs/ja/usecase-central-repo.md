...
多数のサービス間でガイドラインを共有する組織に最適です。

## セットアップ

1. 専用のリポジトリを作成します (例: `my-org/guidelines`)。
2. `kex init` を実行します。
3. GitHub Actions を設定し、`kex generate dist` を実行して `dist/` を GitHub Pages にデプロイします。
...
3. **利用**: 接続されているすべてのエージェントは、即座に新しいルールを参照できます。

## メリット/デメリット

- **メリット**: 唯一の信頼できる情報源 (SSOT)、グローバルな更新が容易。
- **デメリット**: バージョンの乖離 (コードがルールより遅れる可能性がある)、ネットワーク依存。
