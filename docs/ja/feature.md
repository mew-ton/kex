...
検索結果は **スコープの共通部分 (Intersection)** によってフィルタリングされます。以下の条件を満たすドキュメントがヒットします:
- **キーワード** がマッチする。
- かつ (スコープが有効な場合)、クエリのコンテキストと少なくとも1つの **スコープ** を共有している。

これにより、例えば `backend/` ディレクトリで作業中に「アーキテクチャ」のような一般的な用語で検索を行った場合、バックエンド関連のアーキテクチャドキュメントが優先的にヒットするようになります (もちろん、適切にスコープ設定されている必要があります)。また、共通のルートスコープを持つグローバルなドキュメントも取得可能です。
