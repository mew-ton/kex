...
- **型**: `string`
- **説明**: ドキュメントの人間が読めるタイトルです。

### `description` (必須)

- **型**: `string`
- **説明**: ドキュメントの内容の簡潔な要約です。これは AI エージェントが関連するガイドラインを選択するために使用されます。

### `keywords` (必須)

- **型**: `string[]` (文字列のリスト)
- **説明**: 検索インデックスを容易にするためのキーワードのセットです。
...
- **説明**: ガイドラインのライフサイクルステータスです。
  - `adopted`: ガイドラインは有効であり、従う必要があります。
  - `draft`: ガイドラインは作成中です。

### `sources` (オプション)

- **型**: `object[]` (オブジェクトのリスト)
- **説明**: ガイドラインをサポートする外部参照やソースのリストです。
